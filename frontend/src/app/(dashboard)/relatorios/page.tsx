...
export default function RelatoriosPage() {
    ...
    const fetchReportData = async () => {
        ...
        try {
            ...
            const [overviewRes, stockValueRes, purchaseHistoryRes] = await Promise.all([
                ...
                fetch(`${process.env.NEXT_PUBLIC_API_URL}/relatorios/stock-value-by-loja`, { headers }),
            ...
            const stockValueChartData: StockValueByLoja[] = await stockValueRes.json();
            ...
            setStockValueByLoja(stockValueChartData);
    ...
    return (
        ...
        <div className={styles['main-wrapper']}>
            <div className={styles['page-header-relatorios']}>
                <h1 className={styles['page-title-relatorios']}>Relatórios de Gestão</h1>
            </div>

            {error && !isLoading && <p className={`${styles['relatorios-message']} ${styles['relatorios-error']}`}>{error}</p>}
            {isLoading && <p>Carregando relatórios...</p>}

            {!isLoading && !error && (
                <div className={styles['relatorios-container']}>

                    {/* --- 1. Secção Visão Geral (KPIs) --- */}
                    <section className={styles['report-section']}>
                        <h2 className={styles['section-title']}>Visão Geral (Todas as Lojas)</h2>
                        <div className={styles['kpi-grid']}>
                            ...
                            {/* 3. Classe condicional com template literal */}
                            <div className={`${styles['kpi-card']} ${lowStockCount && lowStockCount > 0 ? styles['kpi-warning'] : ''}`}>
                    ...
                    {/* --- 3. Secção Downloads --- */}
                    <section className={styles['report-section']}>
                        <h2 className={styles['section-title']}>Downloads (XLSX)</h2>
                        
                        <div className={styles['download-form']}>
                            <h3 className={styles['download-subtitle']}>Relatório de Controle (Serviços)</h3>
                        ...
                        <div className={styles['download-form']}>
                             <h3 className={styles['download-subtitle']}>Relatório de Estoque por Fornecedor</h3>
                             <button
                                className={styles['btn-secondary']}
                    ...
                    {/* --- 2. Secção Gráficos --- */}
                    <section className={styles['report-section']}>
                        <h2 className={styles['section-title']}>Gráficos</h2>
                        <div className={styles['charts-grid']}>
                            ...
                            <div className={styles['chart-container']}>
                                ...
                                {stockValueByLoja.length > 0 ? (
                                    ...
                                    <p className={styles['no-data-message']}>Sem dados para exibir o gráfico de valor.</p>
                                )}
                            </div>
                            ...
                            <div className={styles['chart-container']}>
                                ...
                                {purchaseHistoryData.length > 0 ? (
                                    ...
                                    <p className={styles['no-data-message']}>Sem dados para exibir o histórico de compras.</p>
                                )}
                            </div>
