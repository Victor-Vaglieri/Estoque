...
let UsersService = class UsersService {
    ...
    async updateLogin(userId, newLogin) {
        try {
            ...
            if (error.code === 'P2002') {
                throw new common_1.UnauthorizedException('Este login já está em uso.');
            }
            throw error;
        }
    }
    async updatePassword(userId, dto) {
        ...
        const isCurrentPasswordValid = await bcrypt.compare(dto.currentPassword, user.senha);
